...
def get_users_with_group_membership():
    r = requests.get(confluence_url + 'group/confluence-users/member?&limit=1000', headers = headers)
    result = r.json()
...
def process(event, context):
    ...
    users_with_their_groups = get_users_with_group_membership()
    ...
    put_data_to_s3_bucket(users_with_their_groups,'Confluence_users_with_groups_membership.json','Uploaded users list with groups membership')
